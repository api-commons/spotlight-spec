...
//
// Usage:
//   node tools/sync-from-spectral.mjs [--src <dir>] [--check]
...
// Recursively delete the runtime-only x-spectral-runtime hook (no validation effect).
function stripRuntime(node) {
  if (Array.isArray(node)) return node.forEach(stripRuntime);
  if (node && typeof node === "object") {
    delete node["x-spectral-runtime"];
...
async function build() {
  ...
  const out = {
    ...
    $comment:
      "GENERATED by tools/sync-from-spectral.mjs from Stoplight Spectral " +
  ...
  if (checkOnly) {
    ...
    if (current !== json) {
      ...
      console.error(`Regenerate with: node tools/sync-from-spectral.mjs  (source: ${origin})`);
      process.exit(1);
    }
